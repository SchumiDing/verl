...
@register("multiroute_nonedistill_cover_rate_cot")
class MultirouteNonedistillCoverRateCoTRewardManager(RewardManagerBase):
    """The reward manager."""
    ...
    async def run_single(self, data: DataProto) -> dict:
        ...
        hitnum = {i:0 for i in range(len(ground_truth))}
        hitloc = []
        for i in range(self.rollout_n):
            data_item = data[i]
        ...
        if sum(finded) > 1:
            cover_score = sum(finded)/len(ground_truth)
        else:
            cover_score = 0
        for i in range(self.rollout_n):
            if hitloc[i] != -1:
                reward_scores[i] = cover_score / hitnum[hitloc[i]]
