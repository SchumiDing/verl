...
# Optional: use Flash_Attn_with_Score_bp_support for attention with score computation (fused kernel, backward support)
_FLASH_ATTN_WITH_SCORE_AVAILABLE = False
_flash_attn_attention_with_scores_backward = None
try:
    _verl_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
    _flash_attn_path = os.path.join(_verl_root, "Flash_Attn_with_Score_bp_support")
    if os.path.isdir(_flash_attn_path):
        import sys

        if _verl_root not in sys.path:
            sys.path.insert(0, _verl_root)
        from Flash_Attn_with_Score_bp_support import (
            attention_with_scores_backward as _flash_attn_attention_with_scores_backward,
        )

        _FLASH_ATTN_WITH_SCORE_AVAILABLE = True
        _flash_attn_attention_with_scores_backward = _flash_attn_attention_with_scores_backward
except Exception:
    pass

# Optional: use Triton topk/bottomk kernel for score aggregation (replaces heavy Python logic)
_TOPK_KERNEL_AVAILABLE = False
_topk_bottomk_sum_fn = None
_TOPK_MAX_KV_LEN = 4096
try:
    from .topk_attn_score_kernel import MAX_KV_LEN as _TOPK_MAX_KV_LEN
    from .topk_attn_score_kernel import topk_bottomk_sum as _topk_bottomk_sum_fn

    _TOPK_KERNEL_AVAILABLE = True
except Exception:
    pass
...
def compute_attention_with_scores_chunked(
    ...
    # When Flash_Attn_with_Score_bp_support is available, use it instead of chunked matmul (single fused kernel)
    if _can_use_flash_attn_with_scores(query_states, key_states, attention_mask, token_mask, cu_seqlens, head_dim):
        return _compute_attention_with_scores_flash(
    ...
    is_packed_sequence = bsz == 1 and token_mask is not None and cu_seqlens is not None and token_mask.shape[0] > 1
    ...
    for chunk_start in range(0, q_len, chunk_size):
        ...
        if token_mask is not None:
                ...
                for b in range(real_bsz):
                    ...
                    if chunk_start < end_idx and chunk_end > start_idx:
                        ...
                        if valid_length >= seq_len_in_sample:
                            ...
                            mask_end = min(
                                valid_length, global_chunk_end - start_idx - (valid_length - seq_len_in_sample)
                            )
            ...
            expanded_chunk_mask = (
                chunk_token_mask.unsqueeze(1).expand(-1, num_heads, -1).reshape(real_bsz, num_heads * chunk_len)
            )
            ...
            if is_packed_sequence:
                ...
                if len(valid_indices) > 0:
                    ...
                    if not use_dynamic_mask and attention_mask is not None:
                        ...
                        chunk_mask_flat = mask_chunk_flat[
                            :, chunk_start * num_heads : (chunk_start + chunk_len) * num_heads, :
                        ]
...
def compute_scores_vectorized_efficient(
    ...
    Uses Triton topk/bottomk kernel when available (CUDA, kv_len <= MAX_KV_LEN), else Python path.
    ...
    # Try Triton kernel path when available (CUDA, kv_len within limit)
    if (
        _TOPK_KERNEL_AVAILABLE
        and _topk_bottomk_sum_fn is not None
        and attn_probs_flat.is_cuda
        and kv_len <= _TOPK_MAX_KV_LEN
    ):
        # Same per-row k as kernel: k = clamp(round(valid_count * pct), 1, valid_count)
        k_top_all = (valid_counts.float() * top_percent).long().clamp(min=1)
        k_bottom_all = (valid_counts.float() * bottom_percent).long().clamp(min=1)
        k_top_all = torch.minimum(k_top_all, valid_counts)
        k_bottom_all = torch.minimum(k_bottom_all, valid_counts)
        total_top_count = k_top_all.sum().to(dtype)
        total_bottom_count = k_bottom_all.sum().to(dtype)
        try:
            topk_sum, bottomk_sum = _topk_bottomk_sum_fn(
                attn_probs_flat, top_percent, bottom_percent, attention_mask_flat
            )
            denom_top = total_top_count * num_layers
            denom_bottom = total_bottom_count * num_layers
            top_mean = (topk_sum / denom_top) if denom_top > 0 else torch.tensor(0.0, device=device, dtype=dtype)
            bottom_mean = (
                (bottomk_sum / denom_bottom) if denom_bottom > 0 else torch.tensor(0.0, device=device, dtype=dtype)
            )
            return top_mean, bottom_mean
        except Exception:
            pass  # fall through to Python path

    # Python path: extract valid tokens and compute top/bottom sums
...
def _can_use_flash_attn_with_scores(
    query_states: torch.Tensor,
    key_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor],
    ...
    """
    Check whether Flash_Attn_with_Score_bp_support can be used for this attention call.
    Flash path supports: CUDA, head_dim in {16,32,64,128}, causal only (no custom mask), no packed sequences.
    """
    if not _FLASH_ATTN_WITH_SCORE_AVAILABLE or _flash_attn_attention_with_scores_backward is None:
        return False
    if not query_states.is_cuda or not key_states.is_cuda:
        return False
    if head_dim not in (16, 32, 64, 128):
        return False
    # Flash kernel only supports causal or no mask; custom attention_mask not supported
    if attention_mask is not None:
        return False
    bsz = query_states.shape[0]
    # Packed sequence (bsz=1 with multiple samples in one sequence) not supported by flash
    if bsz == 1 and token_mask is not None and cu_seqlens is not None and token_mask.shape[0] > 1:
        return False
    return True


def _compute_attention_with_scores_flash(
    query_states: torch.Tensor,
    key_states: torch.Tensor,
    value_states: torch.Tensor,
    top_percent: float,
    ...
    """
    Compute attention and score statistics using Flash_Attn_with_Score_bp_support.
    Returns (attn_output, top_score_means, bottom_score_means).
    Caller must ensure _can_use_flash_attn_with_scores() is True.
    """
    device = query_states.device
    dtype = query_states.dtype
    ...
    # Forward with backward support: (output, scores); scores are pre-softmax (s*sm_scale), masked positions 0
    attn_output, scores = _flash_attn_attention_with_scores_backward(
        query_states,
        key_states,
        value_states,
        causal=True,
    ...
    # Build causal mask for converting scores to probs: invalid (masked) positions must be -inf before softmax
    # Flash convention: valid where (P_SEQ + m) >= n, i.e. n <= P_SEQ + m, with P_SEQ = N - M
    M, N = q_len, kv_len
    P_SEQ = N - M
    # causal_invalid[m, n] = True when n > P_SEQ + m
    causal_invalid = torch.arange(N, device=device, dtype=torch.long).unsqueeze(0) > (
        P_SEQ + torch.arange(M, device=device, dtype=torch.long).unsqueeze(1)
    )
    # scores: (B, H, M, N); masked positions are 0 in flash output
    scores_masked = scores.masked_fill(causal_invalid.unsqueeze(0).unsqueeze(0), float("-inf"))
    attn_probs = F.softmax(scores_masked, dim=-1, dtype=torch.float32).to(dtype)
    # Attention mask for stats: -inf where invalid, 0 elsewhere (same shape as attn_probs for reshape)
    mask_bool = causal_invalid.unsqueeze(0).unsqueeze(0).expand(bsz, num_heads, M, N)
    attention_mask_for_stats = torch.where(
        mask_bool,
        torch.tensor(float("-inf"), device=device, dtype=dtype),
        torch.tensor(0.0, device=device, dtype=dtype),
    )
    attn_probs_per_batch = attn_probs.reshape(bsz, num_heads * q_len, kv_len)
    attention_mask_per_batch = attention_mask_for_stats.reshape(bsz, num_heads * q_len, kv_len)
    if token_mask is None:
        top_mean, bottom_mean = compute_scores_vectorized_efficient(
            attn_probs_per_batch.reshape(-1, kv_len),
            attention_mask_per_batch.reshape(-1, kv_len),
            top_percent,
            bottom_percent,
            num_layers,
        )
        top_score_means = top_mean.unsqueeze(0).expand(bsz).to(device=device, dtype=dtype)
        bottom_score_means = bottom_mean.unsqueeze(0).expand(bsz).to(device=device, dtype=dtype)
    else:
        ...
        if token_mask.shape[1] != q_len:
            sliced_mask = (
                token_mask[:, -q_len:]
                if token_mask.shape[1] >= q_len
                else F.pad(token_mask, (q_len - token_mask.shape[1], 0), value=0)
            )
        else:
            sliced_mask = token_mask
        if sliced_mask.dim() == 1:
            sliced_mask = sliced_mask.unsqueeze(0)
        expanded_mask = sliced_mask.unsqueeze(1).expand(-1, num_heads, -1).reshape(bsz, num_heads * q_len)
        top_list = []
        bottom_list = []
        for b in range(bsz):
            valid_indices = expanded_mask[b].nonzero(as_tuple=True)[0]
            if len(valid_indices) == 0:
                top_list.append(torch.tensor(0.0, device=device, dtype=dtype))
                bottom_list.append(torch.tensor(0.0, device=device, dtype=dtype))
                continue
            ...
            top_list.append(top_mean.to(device=device, dtype=dtype))
            bottom_list.append(bottom_mean.to(device=device, dtype=dtype))
        top_score_means = torch.stack(top_list)
        bottom_score_means = torch.stack(bottom_list)
    return attn_output, top_score_means, bottom_score_means
...
def compute_attention_with_scores(
    ...
    # Try Flash_Attn_with_Score_bp_support first when applicable (fused kernel, supports backward)
    if _can_use_flash_attn_with_scores(query_states, key_states, attention_mask, token_mask, cu_seqlens, head_dim):
        return _compute_attention_with_scores_flash(
            query_states=query_states,
            key_states=key_states,
    ...
    if attention_mask is not None:
        ...
        if attention_mask.shape[1] == 1:
            # Expand to all heads
            attention_mask_per_batch = attention_mask.expand(-1, num_heads, -1, -1).reshape(
                bsz, num_heads * q_len, kv_len
            )
        else:
    ...
    def compute_scores_vectorized(
        attn_probs_flat, valid_mask, is_packed=False, cu_seqlens_tensor=None, attention_mask_flat=None
    ):
        """
        ...
        if bsz == 1 and real_bsz > 1:
            ...
            if cu_seqlens is None:
                if torch.distributed.get_rank() == 0:
                    print("[Warning] cu_seqlens not provided in packed sequence mode. Skipping masking.")
                token_mask = None
            else:
                ...
                if cu_seqlens.shape[0] < real_bsz + 1:
                    if torch.distributed.get_rank() == 0:
                        print(
                            f"[Warning] cu_seqlens length ({cu_seqlens.shape[0]}) < real_bsz+1 ({real_bsz + 1}). Skipping masking."
                        )
                    token_mask = None
                ...
                if token_mask is not None:  # Only proceed if token_mask is still valid
                    ...
                    for b in range(real_bsz):
                        ...
                        if start_idx < 0 or end_idx > q_len or start_idx >= end_idx:
                            if torch.distributed.get_rank() == 0:
                                print(
                                    f"[Warning] Invalid cu_seqlens for sample {b}: start={start_idx}, end={end_idx}, q_len={q_len}. Skipping."
                                )
                            continue
                            ...
                            if start_idx >= 0 and end_idx <= q_len and start_idx < end_idx:
                                ...
                                if start_idx + sample_mask_slice.shape[0] <= q_len:
                                    try:
                                        ...
                                        if torch.distributed.get_rank() == 0:
                                            print(
                                                f"[Warning] Mask assignment failed for sample {b}: "
                    ...
                    if len(valid_indices) > 0:
                        ...
                        if attention_mask_per_batch is not None:
                            valid_attention_mask = attention_mask_per_batch[
                                0, valid_indices, :
                            ]  # Direct attention mask

                        # Create valid positions mask for packed sequence
                        # Pre-compute sample boundaries
                        cu_seqlens_cpu = cu_seqlens.cpu().numpy()
                        valid_positions_mask = torch.zeros(
                            len(valid_indices), kv_len, dtype=torch.bool, device=valid_attn.device
                        )
    ...
    if token_mask is not None:
        ...
        for b in range(bsz):
            ...
            if attention_mask_per_batch is not None:
                valid_attention_mask = attention_mask_per_batch[b, valid_indices, :]
            top_mean, bottom_mean = compute_scores_vectorized(
                valid_attn, None, attention_mask_flat=valid_attention_mask
            )
        ...
        for b in range(bsz):
            ...
            top_mean, bottom_mean = compute_scores_vectorized(
                batch_attn, None, attention_mask_flat=batch_attention_mask
            )
            top_score_means.append(top_mean)
            bottom_score_means.append(bottom_mean)
...
if __name__ == "__main__":
    ...
    assert bottom_scores.shape == (batch_size,), (
        f"Expected bottom_scores shape {(batch_size,)}, got {bottom_scores.shape}"
    )
    ...
    causal_mask_test = torch.triu(
        torch.full((test_seq, test_seq), float("-inf"), device=device, dtype=dtype), diagonal=1
    )
    ...
    print(
        f"  Built-in probs sum (should be ~1.0): min={builtin_probs_sum.min():.6f}, max={builtin_probs_sum.max():.6f}"
    )
    ...
    # Create cu_seqlens
    cu_seqlens_test = torch.tensor(
        [0] + [sum(seq_lens[: i + 1]) for i in range(num_seqs)], device=device, dtype=torch.long
    )
    ...
    for i in range(num_seqs):
        ...
        print(
            f"  Seq {i}: positions {start}-{end - 1}, masked: {token_mask_test[i, start:end].nonzero(as_tuple=True)[0].tolist()}"
        )
    ...
    print("  Normal mode individual:")
    for i, (t, b) in enumerate(zip(normal_top_scores, normal_bottom_scores, strict=False)):
